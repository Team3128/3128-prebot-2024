...
public abstract class SwerveBase1 extends SubsystemBase {
    ...
    private double stillThreshhold, errorThreshold;
    ...
    public SwerveBase1(SwerveDriveKinematics kinematics, Matrix<N3, N1> stateStdDevs, Matrix<N3, N1> visionMeasurementDevs, double stillThreshhold, double errorThreshold, SwerveModuleConfig... configs) {
        ...
        stillCount = 0;
        this.stillThreshhold = stillThreshhold;
        this.errorThreshold = errorThreshold;
    }

    public SwerveBase1(SwerveDriveKinematics kinematics, Matrix<N3, N1> stateStdDevs, Matrix<N3, N1> visionMeasurementDevs, SwerveModuleConfig... configs) {
        this(kinematics, stateStdDevs, visionMeasurementDevs, 2, 20, configs);
    }
    ...
    public void drive(ChassisSpeeds velocity) {
        ...
        for (SwerveModule module : modules) {
            maxError = Math.max(maxError, Math.abs(module.getAbsoluteAngle().getDegrees() - module.getState().angle.getDegrees()));
        }
        if (stillCount >= stillThreshhold / .02 && maxError >= errorThreshold) {
            resetEncoders();
            stillCount = 0;
